    ...
    <div>
            ...
            <el-col>
                <div class="grid-content bg-purple-dark centerClass">
                    <el-input placeholder="按关键字搜索" v-model="treeOption.search" prefix-icon="el-icon-search"
                        ...
                        <template #append>
                            <el-button type="primary" icon="el-icon-plus" @click="handleTreeNodeAdd"></el-button>
                        </template>
                    </el-input>
                </div>
            </el-col>
            <el-col>
                ...
                <el-scrollbar :style="{ height: (useThat.sizeOption.global.height - 100) + 'px' }">
                    ...
                    <el-tree ref="refTree" :data="treeOption.data" highlight-current :props="treeOption.defaultProps"
                        default-expand-all @node-click="handleTreeNodeClick" node-key="id"
                        ...
                        <template #default="{ node, data }">
                            <span class="custom-tree-node">
                                <span> <span v-if="data.kind_type == 2"><span
                                            :style="{ 'color': apiUtil.getTagColor('Fn') }">Fn</span></span> {{
                        node.label }}
                                    <span v-if="data.kind_type == 1">({{ data.key }})</span><span v-else> ({{ data.key
                                        }})</span>
                                </span>
                                <span class="context-menu">
                                    <el-dropdown trigger="click" @command="handleTreeNodeCommand($event, node, data)">
                                        ...
                                        <template #dropdown>
                                            <el-dropdown-menu>
                                                ...
                                                <el-dropdown-item icon="el-icon-plus" command="add" divided
                                                    v-if="data.kind_type == 1">增加函数</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-edit" command="edit"
                                                    v-if="data.kind_type != 1">编辑函数</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-document-copy" command="copy"
                                                    v-if="data.kind_type != 1">复制函数</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-delete" command="delete"
                                                    v-if="data.kind_type != 1">删除函数</el-dropdown-item>
                                            </el-dropdown-menu>
                                        </template>
                                    </el-dropdown>
                                </span>
        ...
        <el-dialog :title="dialogOption.title" v-model="dialogOption.visible" width="40%" append-to-body
            ...
            <el-form ref="refForm" :model="dialogOption.form" label-width="110px"
                ...
                <el-form-item label="函数路径:" prop="key">
...
const dialogOption = reactive({
    ...
    rules: {
        ...
        key: [
            { required: true, message: "请输入函数路径", trigger: "blur" }
        ]
    }
})
...
const handleTreeNodeClick = (node) => {
    ...
    node["componentName"] = "box-magic-function-main"
...
const handleNodeRemove = async (node, data) => {
...
const handleTreeNodeCommand = (command, node, data) => {
    if (command == "addGroup") {
        dialogOptionGroup.form = {
            kind_type: 1
        }
    ...
    if (command == "add") {
        ...
        dialogOption.title = "添加函数"
        return
    }

    if (command == "edit") {
        dialogOption.visible = true;
        dialogOption.title = "编辑函数";
        nextTick(() => {
