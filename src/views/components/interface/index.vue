<template>
    <div>
        <el-row>
            <el-col>
                <div class="grid-content bg-purple-dark centerClass search-nav"
                    style="display: flex;flex-direction: row;">
                    <el-input placeholder="按关键字搜索" v-model="treeOption.search" prefix-icon="el-icon-search"
                        class="searchClass" clearable @update:modelValue="handleSearch" style="flex: 1;">
                    </el-input>
                    <el-button size="small" icon="el-icon-plus" title="添加" @click="handleGroupAdd"></el-button>
                    <el-button size="small" icon="el-icon-refresh" title="刷新" @click="handleToolbarRefresh"></el-button>
                    <el-button size="small" icon="el-icon-switch" title="折叠/展开" @click="handleToolbarFold"></el-button>
                </div>
            </el-col>
            <el-col>
                <el-scrollbar :style="{ height: (useThat.sizeOption.global.height - 100) + 'px' }">
                    <el-tree size="small" ref="refTree" :data="treeOption.data" highlight-current :props="treeOption.defaultProps"
                        ...
                        :filter-node-method="filterNode" v-loading="treeOption.loading">

                        <template #default="{ node, data }">
                            <span class="custom-tree-node">
                                <span>

                                    <span v-if="data.kind_type == 2"><span
                                            :style="{ 'color': apiUtil.getTagColor(data.param?.method) }">{{
                        data.param?.method }}</span></span> {{ node.label }} <span
                                        v-if="data.kind_type == 1">({{ data.key }})</span><span v-else> ({{ data.key
                                        }})</span>
                                        ...
                                        <template #dropdown>
                                            <el-dropdown-menu>
                                                ...
                                                <el-dropdown-item icon="el-icon-plus" command="add" divided
                                                    v-if="data.kind_type == 1">增加接口</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-edit" command="edit"
                                                    v-if="data.kind_type != 1">编辑接口</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-document-copy" command="copy"
                                                    v-if="data.kind_type != 1">复制接口</el-dropdown-item>
                                                <el-dropdown-item icon="el-icon-delete" command="delete"
                                                    v-if="data.kind_type != 1">删除接口</el-dropdown-item>
                                            </el-dropdown-menu>
                                        </template>
                                    </el-dropdown>
                                </span>
        ...
        <el-dialog :title="dialogOption.title" v-model="dialogOption.visible" width="40%" append-to-body
            ...
            <el-form ref="refForm" :model="dialogOption.form" label-width="110px"
                ...
                <el-form-item label="接口路径:" prop="key">
...
const handleToolbarFold = () => {
    treeOption.isExpand = !treeOption.isExpand
    let nodes = refTree.value.store.nodesMap;
    for (const node in nodes) {
        nodes[node].expanded = treeOption.isExpand;
    }
}
...
const handleNodeClick = (node) => {
    ...
    node["componentName"] = "box-magic-interface-main"
...
const handleCommand = (command, node, data) => {
    ...
    if (command == "add") {
        dialogOption.form = {
            ...
            script: ` return "hello api" `
        }
        dialogOption.visible = true;
        dialogOption.title = "添加接口"
        return
    }

    if (command == "edit") {
        dialogOption.visible = true;
        dialogOption.title = "编辑接口";
        nextTick(() => {
